...
def parse_csynth(text: str) -> dict:
    ...
    # ---- Latency + pipeline type ------------------------------------------
    # Data row: |  67|  67|  0.670 us|  0.670 us|  68|  68|  no|
    lat_row = re.search(
    ...
    if lat_row:
        ...
        data["interval_min_cycles"]       = _int(lat_row.group(5))
        data["interval_max_cycles"]       = _int(lat_row.group(6))
        data["pipeline_type"]             = lat_row.group(7).strip()

    # ---- Utilization summary ----------------------------------------------
    # Column order: BRAM_18K | DSP | FF | LUT | URAM
    for label, keys in [
...
def compute_derived(raw: dict) -> dict:
    ...
    single_op_tgt_ns = _latency_ns(op_cycles, clk_target)
    ...
    # Total energy [J] — use estimated clock × lat_max
    ...
    # ADP = area_norm × latency_max_at_estimated_ns
    derived["adp"] = (
    ...
    # EDP = energy_total × latency_max_at_estimated_ns
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "Output JSON path. "
            "Defaults to reports/<PROJECT>/<VERSION>/metrics.json"
        ),
    )
